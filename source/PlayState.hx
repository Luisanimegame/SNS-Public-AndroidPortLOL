...
class PlayState extends MusicBeatState
{
	...
	private var dialogueEndList = CoolUtil.coolTextFile("assets/data/endDialogueList.txt");
	var usesDialogue:Bool = false;
	var usesEndDialogue:Bool = false;
	var doof:DialogueBox;
	var doof2:DialogueBox;

	var fReturn:String; //thanks smokey uwu
	...
	public static var steve:Character;
	...
	public var dialogueEnd:Array<String> = ['sex', 'yup'];

	var halloweenBG:FlxSprite;
	var isHalloween:Bool = false;

	private var chromeAmountLol:Float = 0;

	var hasSteve:Bool = false;
	...
	override public function create()
	{
		...
		if(dialogueList.contains(StringTools.replace(SONG.song," ", "-").toLowerCase())){
			dialogue = CoolUtil.coolTextFile("assets/data/" + StringTools.replace(SONG.song," ", "-").toLowerCase() + "/dialogue.txt");
			usesDialogue = true;
		}
		if(dialogueEndList.contains(StringTools.replace(SONG.song," ", "-").toLowerCase())){
			dialogueEnd = CoolUtil.coolTextFile("assets/data/" + StringTools.replace(SONG.song," ", "-").toLowerCase() + "/endDialogue.txt");
		...
		switch(SONG.stage)
		{
			case 'halloween': 
			{
				...
				var hallowTex = Paths.getSparrowAtlas('bnb_bg','week2');
			...
			case 'limo':
			{
					...
					if(FlxG.save.data.distractions){
						...
						for (i in 0...5)
						{
								var dancer:BackgroundDancer = new BackgroundDancer((370 * i) + 200, bgLimo.y - 430);
					...
					var limoTex = Paths.getSparrowAtlas('limo/booba','week4');
		...
		switch (SONG.gfVersion)
		{
			...
			case 'gf-steve':
				gfVersion = 'gf';
				if (skinList[FlxG.save.data.activeSkin] == 'Swappin')
					hasSteve = true;
			case 'gf-car':
				gfVersion = 'gf-car';
			case 'gf-christmas':
		...
		gf = new Character(400, 130, gfVersion, false, skinList[FlxG.save.data.activeSkin]);
		gf.scrollFactor.set(0.95, 0.95);

		if (hasSteve)
		{
			steve = new Character(375, 350, 'steve', false);
			steve.scrollFactor.set(0.95, 0.95);
		}
		

		dad = new Character(100, 100, SONG.player2, false, skinList[FlxG.save.data.activeSkin]);
		...
		switch (SONG.player2)
		{
			case 'gf':
				if (skinList[FlxG.save.data.activeSkin] == 'Swappin')
					dad = new Character(100, 100, 'gf-tut', false, skinList[FlxG.save.data.activeSkin]);
				else
				{
			...
			case "brakenboss":
				if (skinList[FlxG.save.data.activeSkin] == 'Swappin')
					dad.y += 160;
				else if (skinList[FlxG.save.data.activeSkin] == 'Funkin')
					dad.y += 200;
			case "monster":
		...
		kadeEngineWatermark = new FlxText(4,healthBarBG.y + 50,0,SONG.song + " " + (storyDifficulty == 2 ? "Hard" : storyDifficulty == 1 ? "Normal" : "Easy") + (Main.watermarks ? " - KE " + MainMenuState.kadeEngineVer : "" + " (Saturday Build)"), 16);
		...
		if(!(FlxG.save.data.username == null || FlxG.save.data.password == null))
		{
			var data:Dynamic = {username: FlxG.save.data.username,password: FlxG.save.data.password};

			var req = new haxe.Http('localhost:2000/login');
			...
			req.request(true);
			player = FlxG.save.data.username;
		}else
		{
			player = 'guest';
		}
		...
		healthOverlay = new FlxSprite(0,0).loadGraphic(Paths.image('overlays/healthOverlay'));
		healthOverlay.setGraphicSize(Std.int(FlxG.width));
		healthOverlay.scrollFactor.set();
		healthOverlay.updateHitbox();
		healthOverlay.screenCenter;
		healthOverlay.cameras = [camOverlay];
		add(healthOverlay);
	...
	function startSong():Void
	{
		...
		switch(curSong)
		{
			case 'Roots' | 'Philly Nice' | 'Blammed' | 'Cocoa' | 'Eggnog'| 'R8 It': allowedToHeadbang = true;
	...
	private function generateStaticArrows(player:Int):Void
	{
		for (i in 0...4)
		{
			...
			{
				case 'pixel':
					babyArrow.loadGraphic(Paths.image('weeb/pixelUI/arrowshit'), true, 17, 17);
	...
	override public function update(elapsed:Float)
	{
		...
		//if (curStage == 'tank')
			...
			/*} else {
				ShadersHandler.setBrightness(0.0);
				ShadersHandler.setContrast(1.0);
			}*/
		...
		ShadersHandler.setChrome(chromeAmountLol);

		healthOverlay.alpha = (health / -1) + 1;
		ShadersHandler.setBrightness((health / -1) * -1);
		ShadersHandler.setContrast((health / -1) + 1);
		camHUD.angle = health <= 1 ? (health * -2) + 2 : 0;
		camGame.angle = health <= 1 ? (health * -2) + 2 : 0;
		camOverlay.angle = health <= 1 ? (health * -2) + 2 : 0;
		camHUD.shake(health <= 1 ? (health * -2 + 2) / 800 : 0, 0.01);
		camGame.shake(health <= 1 ? (health * -2 + 2) / 750 : 0, 0.01);
		camOverlay.shake(health <= 1 ? (health * -2 + 2) / 800 : 0, 0.01);
		/*vocals.pan = health <= 1 ? (health * -2) + 2 : 0;
		FlxG.sound.music.pan = health <= 1 ? (health / -1) + 1 : 0;*/
		...
		if (generatedMusic && PlayState.SONG.notes[Std.int(curStep / 16)] != null)
		{
			...
			if(allowedToHeadbang)
			{
				...
				if(gf.animation.curAnim.name == 'danceLeft' || gf.animation.curAnim.name == 'danceRight' || gf.animation.curAnim.name == 'idle')
				{
					...
					switch(curSong)
					{
						...
						case 'R8 It':
						{
							if (curBeat > 32 && curBeat < 90 && curBeat != 62) // I know, modcharts are better but cant prevent it from the FUCKING TRAIN
							{
								if(curBeat % 4 == 2)
								{
									gf.playAnim('cheer');
									if (hasSteve){
										steve.playAnim('cheer');
									}
									triggeredAlready = true;
							...
							if ((curBeat > 126 && curBeat < 154) || (curBeat > 160 && curBeat < 190) || (curBeat > 230 && curBeat < 256))
							{
								if(curBeat % 4 == 1)
								{
									gf.playAnim('cheer');
			...
			if (camFollow.x != dad.getMidpoint().x + 150 && !PlayState.SONG.notes[Std.int(curStep / 16)].mustHitSection)
			{
				...
				switch (dad.curCharacter)
				{
					...
					case 'skearest':
						if (skinList[FlxG.save.data.activeSkin] == 'Funkin') //so sry for the 1093402932309 if statements ;(
							camFollow.y = dad.getMidpoint().y;
					case 'tankman':
						camFollow.y = dad.getMidpoint().y + 65;
						camFollow.x = dad.getMidpoint().x + 400;
						if (skinList[FlxG.save.data.activeSkin] == 'Swappin')
							camFollow.x = camFollow.x - 100;
					case 'yandere':
						if (skinList[FlxG.save.data.activeSkin] == 'Swappin')
						{
		...
		if (curBeat > 0 && !shadersLoaded)
			{
				...
				//filters.push(ShadersHandler.directionalBlur);
			}
